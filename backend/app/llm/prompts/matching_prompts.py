...
JOB_MATCH = PromptTemplate(
    system_prompt=(
        "Role: expert recruiter matching a candidate to a job in the candidate's "
        "field. Use the terminology of the job's own domain — do not assume any "
        "particular industry. Assess fit honestly. score, interview_probability and "
        "salary_confidence are 0-100. transferable_skills are candidate skills that "
        "partly cover a missing requirement. recommended_action is one of apply_now, "
        "upskill_first, stretch, skip. growth_potential is one line. Output ONLY "
        "valid JSON:\n"
...
GAP_ANALYSIS = PromptTemplate(
    system_prompt=(
        "Role: career gap analyst for the candidate's field. Use the terminology of "
        "the job's own domain — do not assume any particular industry. For the "
        "specific job, separate critical_gaps (blockers) from nice_to_have_gaps. "
        "estimated_upskill_weeks is realistic full effort. learning_priority orders "
        "skills to learn first. immediate_focus is the single next skill. Output ONLY "
        "valid JSON:\n"
